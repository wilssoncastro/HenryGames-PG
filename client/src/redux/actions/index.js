...
export function getFilteredVideogames(name, gen, tag, esrb, page, sort, order, limit) {
  return async function (dispatch) {
    let json = await axios(`http://localhost:3001/videogames/filter?name=${name}&gen=${gen}&tag=${tag}&esrb=${esrb}&page=${page}&sort=${sort}&order=${order}&limit=${limit}`);
    return dispatch({
...
export function getNoLimitFilteredVideogames(name, gen, tag, esrb, page, sort, order) {
  return async function (dispatch) {
    let json = await axios(`http://localhost:3001/videogames/filter?name=${name}&gen=${gen}&tag=${tag}&esrb=${esrb}&page=${page}&sort=${sort}&order=${order}`);
    return dispatch({
